...
ttmr2 macro
    ...
    bcf ((PIR1) and 07Fh), 1 ; Se coloca en 0 la bandera
    endm
...
isr:
    btfsc ((PIR1) and 07Fh), 0 ;Se revisa si la bandera de overfl. de TMR1 esta enc
    call int_t1 ;Se llama la subrutina de interrupcion de TMR1
    btfsc ((INTCON) and 07Fh), 2 ;Se revisa si la bandera de overfl. de TMR0 esta enc
    call int_t0 ;Se llama la subrutina de interrupcion de TM
    btfsc ((PIR1) and 07Fh), 1 ;Se revisa si la bandera de overfl. de TMR2 esta enc
...
int_t1:
    ...
    btfss ((STATUS) and 07Fh), 2 ; Si la resta no da 0
...
config_reloj:
    ...
    bcf ((OSCCON) and 07Fh), 4 ; Se configura el IRCF a 110, que son 4MHz de clk
...
config_tmr2:
    ...
    bsf ((T2CON) and 07Fh), 0 ; prescaler de 1:16
    bsf ((T2CON) and 07Fh), 6 ;
    bsf ((T2CON) and 07Fh), 5 ;
    bsf ((T2CON) and 07Fh), 4 ;
    bsf ((T2CON) and 07Fh), 3 ; postcaler de 1:16
    banksel TRISA ;
...
config_int_enable:
    ...
    bsf ((PIE1) and 07Fh), 1 ; Se habilita la interrupcion del timer 2
    bcf ((PIR1) and 07Fh), 1 ; Se limpia la bandera del timer 2
    return
...
overf:
    ...
    btfsc ((STATUS) and 07Fh), 0 ;Si la resta es negativa se salta la instrucc. sig.
