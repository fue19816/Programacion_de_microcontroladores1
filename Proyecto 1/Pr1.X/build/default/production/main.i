...
ttmr0 macro
    ...
    movwf TMR0 ;
    bcf ((INTCON) and 07Fh), 2 ; Se coloca en 0 la bandera del tmr0
    endm
...
ttmr1 macro
    ...
    movlw 0xEE ;
    movwf TMR1L ;
    bcf ((PIR1) and 07Fh), 0 ; Se coloca en 0 la bandera del tmr1
    endm
...
div_dec macro arg1, arg2
    ...
    btfsc ((STATUS) and 07Fh), 0 ;Skip si la resta es negativa
...
overflow macro vari
    ...
    btfsc ((STATUS) and 07Fh), 2 ;Si la resta es negativa se salta la instrucc. sig.
    movlw 10 ;
...
int_tmr0:
    ...
    btfss banderas,5 ;Se revisa si el bit 4 de banderas está encendido
    goto display5 ;Si no, se visualiza el display 5

    btfss banderas,6 ;Se revisa si el bit 4 de banderas está encendido
    goto display6 ;Si no, se visualiza el display 6

    btfss banderas,7 ;Se revisa si el bit 4 de banderas está encendido
...
display5:
    ...
    movf display_var+5, 0 ;Se mueve el valor de la variable display_var+3 a W
    movwf PORTC ;Se mueve W al puerto C
...
display6:
    ...
    movf display_var+6, 0 ;Se mueve el valor de la variable display_var+3 a W
    movwf PORTC ;Se mueve W al puerto C
...
display7:
    ...
    movf display_var+7, 0 ;Se mueve el valor de la variable display_var+3 a W
    movwf PORTC ;Se mueve W al puerto C
...
;----------------------configuración--------------------------------------------
 main:
    ...
    banksel TRISA ;
    clrf TRISA ;
    movlw 01111111B ; Solo los primeros 3 pines de B son entradas
    movwf TRISB ;
...
;Semaforo 1
    ...
    call verde1 ;
    movf cont, 0 ;
    btfsc ((STATUS) and 07Fh), 2 ;Si es igual a cero
    call amarillo1 ;Se llama a la subrutina de amarillo
    movf cont, 0 ;
    btfsc ((STATUS) and 07Fh), 2 ;Se revisa si está en 0 nuevamente
...
;Semaforo 2
    btfss rojo, 1 ; intstrucción para rojo semaforo 2
    call rojo2 ;
    movf cont2, 0 ;
    btfsc ((STATUS) and 07Fh), 2 ;
    call verde2 ;se mete el valor de verde al semaforo
    movf cont2, 0 ;
    btfsc ((STATUS) and 07Fh), 2 ;
    call amarillo2 ;se mete el valor de amarillo
    movf cont2, 0 ;
    btfsc ((STATUS) and 07Fh), 2 ;
    call reinicio2 ;
...
config_reloj:
    ...
    bcf ((OSCCON) and 07Fh), 4 ; Se configura el IRCF a 100, que son 1MHz de clk
    bsf ((OSCCON) and 07Fh), 0 ; Se utiliza el oscilador interno para el clk
    return

config_tmr0:
    banksel TRISA ;
    bcf ((OPTION_REG) and 07Fh), 5 ; Se utiliza el ciclo de reloj interno
    bcf ((OPTION_REG) and 07Fh), 3 ;
    bsf ((OPTION_REG) and 07Fh), 2 ;
    bsf ((OPTION_REG) and 07Fh), 1 ;
    bsf ((OPTION_REG) and 07Fh), 0 ; El valor del prescaler es 111 lo que es 1:256
    ttmr0 ; Se llama al macro de reinicio del tmr0
    return

config_tmr1:
    banksel T1CON ;
    bsf ((T1CON) and 07Fh), 0 ; Se enciende el timer 1
    bcf ((T1CON) and 07Fh), 1 ; Se utiliza el oscilador interno
    bsf ((T1CON) and 07Fh), 5 ;
    bsf ((T1CON) and 07Fh), 4 ; Se utiliza un prescaler de 1:8
...
config_intonch:
    ...
    bcf ((INTCON) and 07Fh), 0 ; Se limpia la bandera que registra el cambio en un
       ; de los puertos
    return

config_int_enable:
    bsf ((INTCON) and 07Fh), 7 ; INTCON
    bsf ((INTCON) and 07Fh), 3 ; Se habilita la interrupción del puerto B
    bcf ((INTCON) and 07Fh), 0 ; Se habilita la iinterrupción cuando el puerto B,
       ; cuando esta recibe un cambio
    bsf ((INTCON) and 07Fh), 5 ; Se habilita la interrupción del timer 0
    bcf ((INTCON) and 07Fh), 2 ; Se habilita la inte. cuando la bandera del TMR0
    bsf ((INTCON) and 07Fh), 6 ; Se habilita las interrupciones de perifericos
    bsf ((PIE1) and 07Fh), 0 ; Se habilita la interrupcion del timer 1
    bcf ((PIR1) and 07Fh), 0 ; Se limpia la bandera del timer 1
    return
...
rojo1:
    btfsc rojo, 0 ;
    return
    movf temp1, 0 ;contador 2
    addwf cont, 1 ;
    movf temp2, 0 ;contador 3
    addwf cont, 1 ;
    movlw 6 ;
    addwf cont, 1 ;
    bsf rojo, 0 ;
...
rojo2:
    ...
    movf temp, 0 ;
    movwf cont2 ;
    movlw 3 ;Se suma el valor del semaforo de amarillo
    addwf cont2, 1 ;
    bsf rojo, 1 ;
    bsf prim_vez, 0 ;
    goto leds ;
...
preparar_display:
    prep_display vard, display_var
    prep_display varu, display_var+1
    prep_display vard1, display_var+2
    prep_display varu1, display_var+3
    prep_display vard2, display_var+4
...
verde_tit1:
    ...
    movlw 3 ;
    subwf cont,0 ;
    btfsc ((STATUS) and 07Fh), 2 ;Si la variable cont es igual a 3
    bsf tit_verd, 0 ;Se pone en 1 el bit 0 de tit_verd
    movlw 1 ;
    subwf cont, 0 ;
    btfsc ((STATUS) and 07Fh), 2 ;Si la variable cont es igual a 0
...
;var, var2 y var3 son las que se van a incrementar en cada uno de los estados
;cont y los otros son los que se van a decrementar para el valor de los semaforos
;vd es para mostrar el valor en el display de configuracion
;vd1,
;se debe de agregar el registro rojo y amarillo
